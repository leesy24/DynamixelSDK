...
int main()
{
  ...
  uint8_t dxl_read_value_8bits = 0;               // Read value of 8bits
  uint16_t dxl_read_value_16bits = 0;             // Read value of 16bits
  ...
  // Set Torque limit
  write2ByteTxRx(port_num, PROTOCOL_VERSION, DXL_ID, ADDR_MX_TORQUE_LIMIT, TORQUE_LIMIT);
  ...
  // Read Torque limit
  dxl_read_value_16bits = read2ByteTxRx(port_num, PROTOCOL_VERSION, DXL_ID, ADDR_MX_TORQUE_LIMIT);
  if ((dxl_comm_result = getLastTxRxResult(port_num, PROTOCOL_VERSION)) != COMM_SUCCESS)
  {
    printf("%s\n", getTxRxResult(PROTOCOL_VERSION, dxl_comm_result));
  }
  ...
  // Read Torque Enable
  dxl_read_value_8bits = read1ByteTxRx(port_num, PROTOCOL_VERSION, DXL_ID, ADDR_MX_TORQUE_ENABLE);
  if ((dxl_comm_result = getLastTxRxResult(port_num, PROTOCOL_VERSION)) != COMM_SUCCESS)
  {
    printf("%s\n", getTxRxResult(PROTOCOL_VERSION, dxl_comm_result));
  }
